...
export function CaseStudyView({ caseStudy }: CaseStudyViewProps): React.ReactElement {
  ...
  }, [caseStudy.steps, currentStepId]);

  const isOutcome = currentStepId === 'outcome';
  const outcome = caseStudy.outcome;

  const handleChoice = useCallback((nextStepId: string) => {
