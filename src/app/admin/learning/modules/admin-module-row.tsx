...
interface AdminModuleRowProps {
  module: { id: string; title: string; slug: string; lesson_count: number };
}
...
export function AdminModuleRow({ module }: AdminModuleRowProps): React.ReactElement {
  ...
  return (
    <tr className="border-b border-black/5 hover:bg-slate-50/50">
      <td className="px-4 py-3 font-medium text-slate-800">{module.title}</td>
      <td className="px-4 py-3 text-slate-600">{module.slug}</td>
      <td className="px-4 py-3 text-slate-600">{module.lesson_count}</td>
      <td className="px-4 py-3">
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" asChild>
            <Link href={`/admin/learning/modules/${module.id}/edit`}>Edit</Link>
          </Button>
          <Button variant="outline" size="sm" asChild>
            <Link href={`/admin/learning/modules/${module.id}/lessons`}>Lessons</Link>
          </Button>
